...
import { currentBranch, createBranch, stageAll, hasStagedChanges, commit, commitEmpty } from '../git.ts';
...
program
  ...
  .action(async (branchArg: string | undefined, opts: { all?: boolean; message?: string }) => {
    ...
    if (!hasStagedNow && !branchArg && !opts.message) {
      cancel('Nothing staged. Stage changes or use -a to stage all working changes.');
      process.exit(1);
    }
    ...
    if (!message) {
      if (isEmpty) {
        ...
        const input = await text({
          ...
          validate: (v) => (v.trim() ? undefined : 'Commit message is required'),
        });
        if (isCancel(input)) { cancel(); process.exit(0); }
